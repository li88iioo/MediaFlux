...
class FakeCleanupClient:
    def __init__(self):
        ...
        self.directories = {
            ...
            "source": [
                GuangYaFile(
                    "empty", "空目录", True, parent_id="source", etag="e1", updated_at=1
                ),
                GuangYaFile(
                    "residual", "a", True, parent_id="source", etag="e2", updated_at=2
                ),
                GuangYaFile(
                    "keep",
                    "保留元数据",
...
class GuangYaResidualCleanupTests(unittest.TestCase):
    def setUp(self):
        ...
        self.patches = [
            ...
            mock.patch.object(
                actions,
                "_configured_sources",
                return_value=[{"id": "source", "name": "整理源"}],
            ),
        ]
    ...
    def test_agent_preview_and_durable_submission(self):
        ...
        with mock.patch.object(actions, "GuangYaClient", return_value=client):
            ...
            confirmation, fingerprint = actions.prepare_guangya_cleanup_confirmation(
                {}, context
            )
            manager = mock.Mock()
    ...
    def test_persisted_preview_rejects_non_list_public_fields(self):
        ...
        value.update({"sample_directories": [], "review_summaries": "not-a-list"})
    ...
    def test_keep_revision_invalidates_previous_confirmation_context(self):
        ...
        with mock.patch.object(actions, "GuangYaClient", return_value=client):
            ...
            _confirmation, previous_context = (
                actions.prepare_guangya_cleanup_confirmation({}, context)
            )
    ...
    def test_user_keep_decision_overrides_previous_quarantine_selection(self):
        ...
        vetoed = cleanup.revise_cleanup_plan(
            ...
            decisions=[
                {"candidate_number": 1, "action": "keep", "reason": "用户明确要求保留"}
            ],
        )
        ...
        cleanup.confirm_cleanup_plan(
            vetoed["plan_id"], owner="owner", expected_fingerprint=vetoed["fingerprint"]
        )
        result = cleanup.execute_cleanup_plan(
            {
                "version": 1,
