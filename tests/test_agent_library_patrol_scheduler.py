...
class AgentLibraryPatrolConfigTests(unittest.TestCase):
    ...
    def test_config_accepts_bounded_values_and_reloads_scheduler(self):
        ...
        with (
            patch("app.routes.api.config.get", return_value=""),
...
class AgentLibraryPatrolDatabaseTests(IsolatedDatabaseTestCase):
    ...
    def test_singleton_claim_and_generation_cas(self):
        ...
        self.assertTrue(
            db.update_agent_library_patrol(
                status="pending",
                outcome="up_to_date",
                attempts=0,
    ...
    def test_stale_lease_is_reclaimed_and_old_worker_cannot_finish(self):
        ...
        new_job = db.claim_due_agent_library_patrol(
            current_time="2026-08-03 12:31:00", stale_before="2026-08-03 12:01:00"
        )
    ...
    def test_reschedule_moves_non_running_task_but_not_active_lease(self):
        ...
        self.assertTrue(
            db.reschedule_agent_library_patrol(next_run_at="2026-08-03 12:00:00")
        )
        self.assertEqual(
            db.get_agent_library_patrol()["next_run_at"], "2026-08-03 12:00:00"
        )
        running = db.claim_due_agent_library_patrol(current_time="2026-08-03 12:00:00")
        self.assertIsNotNone(running)
        self.assertFalse(
            db.reschedule_agent_library_patrol(next_run_at="2026-08-03 11:00:00")
        )
    ...
    def test_notification_outbox_claim_retry_and_sent_use_generation_cas(self):
        with db.get_conn() as conn:
            conn.execute(
                "INSERT INTO agent_library_patrol_notification_outbox(result_revision,fingerprint,outcome,payload_json,next_attempt_at,created_at,updated_at) VALUES(1,?,'updates_available','{}',?,?,?)",
                (
                    "a" * 64,
                    "2026-08-03 12:00:00",
                    "2026-08-03 12:00:00",
        ...
        self.assertIsNone(
            db.claim_due_agent_library_patrol_notification(
                current_time="2026-08-03 12:00:00"
            )
        )
        self.assertTrue(
    ...
    def test_retention_purges_only_expired_terminal_notifications(self):
        ...
        with db.get_conn() as conn:
            for revision, status, sent_at, updated_at in rows:
                conn.execute(
                    "INSERT INTO agent_library_patrol_notification_outbox(result_revision,fingerprint,outcome,payload_json,status,next_attempt_at,sent_at,created_at,updated_at) VALUES(?,?,?,'{}',?,?,?,?,?)",
                    (
                        revision,
                        f"{revision:064d}",
                        "updates_available",
        ...
        recovered = db.claim_due_agent_library_patrol_notification(
            current_time="2026-08-03 12:00:00", stale_before="2026-08-03 11:58:00"
        )
    ...
    def test_init_recovers_interrupted_notification_sender(self):
        with db.get_conn() as conn:
            conn.execute(
                "INSERT INTO agent_library_patrol_notification_outbox(result_revision,fingerprint,outcome,payload_json,status,lease_generation,next_attempt_at,created_at,updated_at) VALUES(1,?,'updates_available','{}','sending',1,?,?,?)",
                (
                    "b" * 64,
                    "2026-08-03 12:00:00",
                    "2026-08-03 12:00:00",
    ...
    def test_continuation_progress_survives_database_recovery(self):
        ...
        self.assertTrue(
            db.continue_agent_library_patrol(
    ...
    def test_continuation_rejects_rollback_or_unmarked_stall(self):
        ...
        claimed = db.claim_due_agent_library_patrol(current_time="2026-08-03 12:00:01")
        self.assertFalse(
            db.continue_agent_library_patrol(
                expected_lease_generation=claimed["lease_generation"],
...
class AgentLibraryPatrolSchedulerTests(IsolatedDatabaseTestCase):
    ...
    def test_disabled_scheduler_does_not_create_or_execute_patrol(self):
        ...
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=executor, clock=self.clock
        )
    ...
    def test_continuation_persists_cursor_and_accumulates_final_projection(self):
        ...
        executor = Mock(side_effect=[(first, 30000), (second, 10000)])
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=executor, clock=self.clock
        )
        with self._config():
        ...
        executor.assert_has_calls(
            [
                ...
                unittest.mock.call(
                    {"as_of": "2026-08-03", "max_series": 50, "after_tmdb_id": "12345"}
                ),
            ]
        )
        row = db.get_agent_library_patrol()
    ...
    def test_stalled_first_item_retries_then_skips_as_inconclusive(self):
        ...
        for _ in range(3):
            ...
            stalled_batches.append((result, 30000))
        ...
        executor = Mock(side_effect=[*stalled_batches, (terminal, 10000)])
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=executor, clock=self.clock
        )
        with self._config():
    ...
    def test_reload_keeps_continuation_progress(self):
        ...
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=Mock(return_value=(first, 30000)), clock=self.clock
        )
        with self._config():
            self.assertEqual(scheduler.run_once(), 1)
            scheduler.reload()
    ...
    def test_a_b_a_transitions_each_enqueue_a_new_revision(self):
        ...
        results[1].data.update(
            {"updates_available_count": 0, "missing_episode_count": 0, "findings": []}
        )
    ...
    def test_notification_disable_discards_backlog(self):
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=Mock(return_value=(_patrol_result(), 10)), clock=self.clock
        )
        with self._config(notify=True):
            scheduler.run_once()
    ...
    def test_notification_disabled_after_claim_does_not_start_sender(self):
        ...
        with (
            patch.multiple(
                "app.modules.agent_library_patrol_scheduler.config", get_bool=get_bool
            ),
            patch(
                "app.modules.agent_library_patrol_scheduler.db.claim_due_agent_library_patrol_notification",
    ...
    def test_inconclusive_result_retries_without_claiming_complete(self):
        ...
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=Mock(return_value=(result, 20)), clock=self.clock
        )
        with self._config():
            self.assertEqual(scheduler.run_once(), 1)
        row = db.get_agent_library_patrol()
    ...
    def test_exception_after_continuation_preserves_cursor_for_retry(self):
        ...
        executor = Mock(side_effect=[(first, 30000), RuntimeError("temporary")])
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=executor, clock=self.clock
        )
        with self._config():
    ...
    def test_exception_stores_only_exception_type(self):
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=Mock(
                side_effect=RuntimeError("token=SECRET /volume/private")
            ),
            clock=self.clock,
        )
        with self._config():
    ...
    def test_status_tool_returns_queryable_snapshot_without_triggering_download(self):
        ...
        with self._config():
            scheduler.run_once()
        with patch(
            "app.agent.library_patrol_status.config.get_bool", return_value=True
        ):
        ...
        self.assertEqual(
            result.data["findings"][0]["missing_sample"][0], {"season": 2, "episode": 3}
        )
    ...
    def test_projection_rejects_uncontrolled_status_and_sensitive_title(self):
        ...
        scheduler = AgentLibraryPatrolScheduler(
            audit_executor=Mock(return_value=(result, 10)), clock=self.clock
        )
        with self._config():
            scheduler.run_once()
        row = db.get_agent_library_patrol()
    ...
    def test_tampered_projection_fails_closed(self):
        ...
        with db.get_conn() as conn:
            conn.execute(
                "UPDATE agent_library_patrol SET outcome='updates_available',projection_json=?,last_finished_at=? WHERE patrol_key='default'",
                (
