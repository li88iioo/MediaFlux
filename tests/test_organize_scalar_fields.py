...
class OrganizeScalarFieldTests(unittest.TestCase):
    ...
    def test_logs_ui_renders_escaped_skip_reasons_in_list_and_detail(self):
        ...
        self.assertIn("class=\"organize-summary-alert ${data.status==='skipped'?'is-skipped':'is-error'}\"", template)
        self.assertIn("<p>${_esc(data.error)}</p>", template)
        ...
        self.assertIn(".organize-summary-alert", styles)
        self.assertIn(".organize-item, .organize-operation, .organize-delete-audit { display: grid; grid-template-columns: auto minmax(0, 1fr) auto", styles)
