...
class IndexerSiteConfigApiTests(IsolatedDatabaseTestCase):
    ...
    def test_natural_language_enable_search_select_second_qb_and_confirm(self):
        ...
        with patch("app.agent.tools.search_resources", return_value=searched) as search, patch(
            "app.agent.tools.preview_submit_resource", return_value=preview
        ) as preview_submit, patch(
            "app.agent.tools.submit_confirmation_context",
            return_value="resource:test:second:qb",
        ), patch(
            "app.agent.tools.submit_resource", return_value=accepted
        ) as submit, patch(
            ...
            self.assertEqual(
                selected_body["confirmation"]["tool"], "indexer.submit_resource"
            )
