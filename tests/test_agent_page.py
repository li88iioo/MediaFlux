...
class AgentPageTests(InitializedWebTestCase):
    ...
    def test_authenticated_agent_renders_stable_semantic_shell(self):
        ...
        self.assertIn("/static/css/agent.css?v=20260901c", response.text)
        self.assertIn("/static/js/agent.js?v=20260901c", response.text)
    ...
    def test_agent_frontend_renders_workspace_overview_and_draft_workflows(self):
        ...
        self.assertIn("/api/agent/tools/${encodeURIComponent(action.tool)}", source)
        self.assertIn("/api/agent/actions/${encodeURIComponent(action.tool)}/prepare", source)
        self.assertIn("confirmationFollowupFromPayload", source)
    ...
    def test_agent_history_mobile_and_accessibility_contracts(self):
        ...
        self.assertIn("viewport.addEventListener('resize'", source)
        self.assertIn("window.addEventListener('pageshow', bindVisualViewport)", source)
        self.assertIn("cancelVisualViewportFrame();", source)
        self.assertIn("historyRail?.querySelector('[data-agent-history-close]')", source)
        ...
        self.assertIn("function actionPlanFromPayload(payload)", source)
        self.assertIn("actionPlan?.plan_id", source)
        self.assertIn("renderActionPlan(actionPlan, payload.tool_call, payload)", source)
        self.assertIn("['操作对象', plan.target]", source)
        self.assertIn("['将会发生', plan.impact]", source)
        ...
        self.assertIn("agent_runtime_disabled", source)
        self.assertIn("restoreRetryableActionPlan(card, confirmButton, cancelButton)", source)
        self.assertIn("card.dataset.planExpiresAt", source)
