...
class ReleaseMetadataTests(unittest.TestCase):
 ...
 def test_spdx_sbom_contains_python_dependencies(self):
  with tempfile.TemporaryDirectory() as d:
   r=Path(d); req=r/'requirements.txt'; req.write_text('fastapi==0.141.1 \\\n    --hash=sha256:' + 'a' * 64 + '\n# x\nuvicorn==0.52.3 \\\n    --hash=sha256:' + 'b' * 64 + '\n'); out=r/'SBOM.spdx.json'; S.generate_sbom([req],out,'https://example/sbom/1'); p=json.loads(out.read_text()); self.assertEqual(p['spdxVersion'],'SPDX-2.3'); self.assertEqual(p['name'],'MediaFlux declared Python dependencies'); self.assertEqual([(x['name'], x['versionInfo']) for x in p['packages']],[('fastapi','0.141.1'),('uvicorn','0.52.3')])
