...
这些规则同时作用于光鸭、本地、Web、Telegram 与定时任务。命名与媒体规格补全采用固定契约，不再提供模板或开关：电影始终建立独立 `{tmdb-ID}` 目录，剧集始终建立 `Season NN` 目录；本地文件直接使用 ffprobe，光鸭文件通过 signedURL/直链探测并缓存成功结果。探测失败不会阻断整理。
