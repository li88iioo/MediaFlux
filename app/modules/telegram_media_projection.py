...
def _event_block(event: NotificationEvent) -> str:
    lines = [str(event.title or "媒体入库")]
    for label, value in event.fields:
        if value in (None, ""):
            continue
        field_label = (
            decorate_field_label(label)
            if event.field_emojis else str(label or "").strip()
        )
        lines.append(f"{field_label}  {value}")
    lines.extend(str(line) for line in event.lines if line not in (None, ""))
    if event.footer not in (None, ""):
        lines.append(str(event.footer))
    return "\n".join(lines)
