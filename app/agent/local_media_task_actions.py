...
class LocalMediaAgentContextStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._tasks: OrderedDict[str, tuple[float, tuple[_TaskRef, ...]]] = OrderedDict()
        self._inspections: OrderedDict[str, tuple[float, tuple[_InspectionRef, ...]]] = OrderedDict()
        self._next_inspection: dict[str, int] = {}

    def set_repository(
        self, repository: AgentSessionContextRepository | None
    ) -> None:
        with self._lock:
            self._repository = repository
            ...
            updated = self._mutate_persisted_locked(
                ...
                lambda current: (
                    ...
                    current[1] if current is not None else (
                        self._inspections.get(owner_key, (0.0, ()))[1]
                    ),
                    current[2] if current is not None else self._next_inspection.get(owner_key, 0),
                ),
            )
        ...
        *,
        ...
        with self._lock:
            ...
            def append_inspection(
                current: tuple[tuple[_TaskRef, ...], tuple[_InspectionRef, ...], int] | None,
            ) -> tuple[tuple[_TaskRef, ...], tuple[_InspectionRef, ...], int]:
                current_tasks = current[0] if current is not None else (
                    self._tasks.get(owner_key, (0.0, ()))[1]
                )
                current_inspections = current[1] if current is not None else (
                    self._inspections.get(owner_key, (0.0, ()))[1]
                )
                next_number = current[2] if current is not None else (
                    self._next_inspection.get(owner_key, 0)
                )
    ...
    def inspection(self, *, owner: str, number: int) -> _InspectionRef | None:
        ...
        with self._lock:
            ...
            if entry is None:
                return None
            selected = next((item for item in entry[1] if item.number == int(number)), None)
    ...
    def _restore_locked(self, owner: str, *, required: str) -> None:
        ...
        try:
            ...
            logger.warning(
                "本地媒体 Agent 上下文恢复失败 type=%s", type(exc).__name__
            )
            return
        if persisted is None:
            return
    ...
    def _mutate_persisted_locked(
        ...
        repository = self._repository
        mutate = getattr(repository, "mutate_latest", None) if repository is not None else None
    ...
    def _decode_payload(
        ...
        if not isinstance(payload, dict) or set(payload) != {
            "version", "tasks", "inspections", "next_inspection"
        ...
        if not isinstance(raw_tasks, list) or not isinstance(raw_inspections, list):
            return None
        if len(raw_tasks) > _MAX_TASK_NUMBER or len(raw_inspections) > self.max_inspections_per_owner:
            return None
        tasks: list[_TaskRef] = []
        ...
        for raw in raw_inspections:
            if not isinstance(raw, dict) or set(raw) != {
                "number", "task", "inspection_id", "digest"
            }:
                return None
            ...
            if (
                ...
                return None
            inspections.append(
                _InspectionRef(number, task, inspection_id, digest)
            )
    ...
    def _decode_task(raw: Any) -> _TaskRef | None:
        if not isinstance(raw, dict) or set(raw) != {
            "task_id", "version", "status", "source_id"
        }:
            return None
...
def _require_owner(context: ToolContext) -> str:
    ...
    if not owner:
        raise AgentToolError("本地媒体任务操作需要已登录会话", code="precondition_failed")
    return owner


def _strict_number(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or not 1 <= value <= _MAX_TASK_NUMBER:
...
def _task_public(task: Any, number: int) -> dict[str, Any]:
    return {
        ...
        "media_type": str(task.media_type) if str(task.media_type) in {"movie", "tv"} else "unknown",
        "season": int(task.season_override) if task.season_override is not None else None,
        "episode": int(task.episode_override) if task.episode_override is not None else None,
        "trigger": str(task.trigger) if str(task.trigger) in {"qb_completed", "scan", "manual"} else "unknown",
...
def list_local_media_task_summaries(
    ...
    return ToolResult(
        ...
        f"已列出 {len(items)} 个本地媒体任务" if items else "当前范围内没有本地媒体任务",
        data={
        ...
        suggestions=["可继续说：检查本地媒体任务 1，或重试本地媒体任务 1。"] if items else [],
    )
...
def _current_task(owner: str, task_number: int) -> tuple[_TaskRef, Any]:
    ...
    if ref is None:
        raise AgentToolError("任务序号不存在或已过期，请先重新列出本地媒体任务", code="precondition_failed")
    ...
    if task is None or (
        ...
        raise AgentToolError("本地媒体任务状态已变化，请重新列出任务", code="precondition_failed")
    return ref, task


def inspect_local_media_task(arguments: dict[str, Any], context: ToolContext) -> ToolResult:
    owner = _require_owner(context)
    ...
    try:
        ...
        raise AgentToolError("暂时无法检查该任务，请确认源文件仍可访问", code="precondition_failed") from None
    ...
    return ToolResult(
        ...
        data={
            ...
            "media_type": str(raw.get("task_media_type") or task.media_type or "unknown"),
            ...
            "parsed_season": raw.get("parsed_season") if isinstance(raw.get("parsed_season"), int) else None,
            "parsed_episode": raw.get("parsed_episode") if isinstance(raw.get("parsed_episode"), int) else None,
            "cloud_write": False,
            "expires_in_seconds": _context_store.ttl_seconds,
        },
        evidence=[Evidence(
        ...
        suggestions=[f"如需查看整理匹配预览，请说：预览本地媒体检查 {inspection_number}。"],
    )
...
def _current_inspection(owner: str, number: int) -> tuple[_InspectionRef, Any]:
    ...
    if ref is None:
        raise AgentToolError("检查序号不存在或已过期，请重新检查任务", code="precondition_failed")
    ...
    if task is None or (
        ...
        raise AgentToolError("任务或源文件状态已变化，请重新检查", code="precondition_failed")
    return ref, task


def preview_local_media_task(arguments: dict[str, Any], context: ToolContext) -> ToolResult:
    owner = _require_owner(context)
    ...
    for item in (raw.get("candidates") if isinstance(raw.get("candidates"), list) else [])[:5]:
        if not isinstance(item, dict):
            continue
        candidates.append({
    ...
    return ToolResult(
        ...
        data={
            ...
            "preview_status": status if status in {"planned", "requires_manual"} else "inconclusive",
...
def prepare_retry_local_media_task(
    ...
    if task.status not in _RETRYABLE:
        raise AgentToolError("只有失败或待人工确认的任务可以重试", code="precondition_failed")
    return ToolResult(
        True,
...
def retry_local_media_task_confirmed(
    ...
    return ToolResult(
        ...
        data={"operation": "retry", "task_number": task_number, "affected": 1, "runtime_refreshed": True},
        evidence=[Evidence(
            "sqlite:local_media_tasks",
...
def _bound_task_scope(task: Any) -> dict[str, Any]:
    if task.status != "completed":
        raise AgentToolError("只有已完成整理的任务可以刷新或核验媒体库", code="precondition_failed")
    items = [
        item for item in db.list_local_media_task_items(task.id, owner=_WORKSPACE_OWNER)
    ...
    if not items or not targets:
        raise AgentToolError("任务没有可验证的已绑定媒体库目标", code="precondition_failed")
    resolved: list[tuple[Any, Path]] = []
    for item in items:
        target_path = Path(str(item["target_path"])).expanduser().resolve(strict=False)
        matches = [target for target in targets if _path_within(target_path, Path(target.path))]
        if not matches:
            raise AgentToolError("任务目标未绑定到明确媒体库", code="precondition_failed")
        longest = max(len(str(Path(target.path).resolve(strict=False))) for target in matches)
        winners = [
            target for target in matches
            if len(str(Path(target.path).resolve(strict=False))) == longest
        ]
        if len(winners) != 1:
            raise AgentToolError("任务目标媒体库绑定存在歧义", code="precondition_failed")
    ...
    if len(keys) != 1:
        raise AgentToolError("一个任务涉及多个媒体库，Agent 不会批量猜测刷新范围", code="precondition_failed")
    ...
    if provider not in {"jellyfin", "emby"} or not (
        ...
        raise AgentToolError("任务目标没有有效的媒体服务器与媒体库绑定", code="precondition_failed")
    profiles = {
    ...
    if profile is None:
        raise AgentToolError("绑定的媒体服务器当前未启用或未配置", code="precondition_failed")
