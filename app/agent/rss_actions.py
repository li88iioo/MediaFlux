...
def get_rss_recent_activity(_arguments: dict[str, Any]) -> ToolResult:
    ...
    try:
        total_downloaded = db.count_rss_downloaded_entries_since(
            db.now(), hours=24
        )
    except Exception as exc:
...
def diagnose_rss(_arguments: dict[str, Any]) -> ToolResult:
    try:
        ...
        return ToolResult(
            ...
            data=_empty_data(),
            evidence=[Evidence("rss_database", "尝试读取本地 RSS 状态；未刷新订阅或提交下载。", _now())],
            suggestions=["请检查本地数据库状态后重试。"],
            error="RSS 诊断当前不可用。",
        )
    ...
    categories = {
        ...
        "unknown_or_inconsistent": max(0, int(entries.get("unknown_or_inconsistent") or 0)),
    ...
    if categories["stale_submitting"]:
        suggestions.append("存在长期停留在提交中的条目，请核对下载后端后再决定是否重试。")
    if categories["aged_pending_backlog"]:
        suggestions.append("存在超过 24 小时的待处理条目，请检查自动下载策略或进行人工处理。")
    if categories["unknown_or_inconsistent"]:
        suggestions.append("发现状态不一致的 RSS 条目，建议先在管理页核对。")
    if categories["invalid_last_refreshed_at"]:
        suggestions.append("发现无法解析的上次刷新时间；系统会将其视为到期并在下次刷新后修复。")
    if categories["cron_configured_but_not_scheduled"]:
        suggestions.append("当前 RSS 调度按刷新间隔运行；仅填写 Cron 不会启用自动刷新。")
    ...
    return ToolResult(
        ...
        evidence=[Evidence("rss_database", "读取本地 RSS 聚合状态；未访问订阅源、刷新订阅或提交下载。", _now())],
        suggestions=suggestions,
    )
