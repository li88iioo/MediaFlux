...
def prepare_clear_preferences(
    ...
    return ToolResult(
        True,
        "confirmation_required",
        "确认后将清除当前会话保存的媒体偏好并恢复默认值",
        data={
            "current": _public_preferences(current),
            "defaults": default_media_preferences(),
        },
...
def get_today_summary(_arguments: dict[str, Any], _context: ToolContext) -> ToolResult:
    ...
    return ToolResult(
        True,
        "completed" if summary["event_count"] else "empty",
        "已汇总今天的媒体内容动态"
...
def set_subscription_notification_rule_confirmed(
    ...
    if not secrets.compare_digest(
        ...
        raise AgentToolError(
            "通知规则或订阅已变化，请重新预检", code="confirmation_stale"
        )
    ...
    return ToolResult(
        True,
        "completed",
        f"媒体订阅 {number} 的通知规则已更新",
        data={
            "operation": "set_notification_rule",
            "subscription_number": number,
            "affected": 1,
...
def reset_subscription_notification_rule_confirmed(
    ...
    return ToolResult(
        True,
        "completed",
        f"媒体订阅 {number} 已恢复默认通知规则",
        data={
            "operation": "reset_notification_rule",
            "subscription_number": number,
            "affected": 1,
        },
    )
