...
def workspace_search_arguments(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if raw_sections is None:
        ...
        if not isinstance(raw_sections, list) or not raw_sections or len(raw_sections) > len(_SECTION_ORDER):
...
def _empty_section(name: str, status: str = "empty") -> dict[str, Any]:
    return {"source": name, "status": status, "returned": 0, "truncated": False, "items": []}
    ...
    for source in sources:
        ...
        for item in source.get("items", []):
            ...
            items.append({
                ...
                "media_type": _safe_status(item.type, {"movie", "series", "episode", "season", "video"}),
...
def _database_section(
    ...
    try:
        ...
        logger.warning("Agent 工作区搜索来源不可用 source=%s type=%s", name, type(exc).__name__)
...
def _rss_item(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "status": _safe_status(row.get("status"), {"pending", "submitting", "downloaded", "failed", "skipped"}),
...
def _download_item(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source": _safe_status(row.get("source"), {"qb", "guangya", "both", "download"}),
        "status": _safe_status(
            row.get("status"),
            {"pending", "submitting", "submitted", "processing", "downloading", "success", "completed", "failed", "partial"},
        ),
...
def _organize_item(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "media_type": _safe_status(row.get("media_type"), {"movie", "tv", "series", "episode", "anime"}),
        ...
        "status": _safe_status(
            row.get("status"),
            {
                "planned", "success", "failed", "partial_failed", "skipped", "interrupted",
                "reorganizing", "returning", "reverting", "deleting", "reverted", "revert_failed", "deleted",
            },
        ),
...
def _local_media_item(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "status": _safe_status(
            row.get("status"),
            {"waiting_stable", "recognizing", "requires_manual", "planned", "moving", "verifying", "refreshing", "completed", "rolling_back", "failed"},
        ),
...
def search_workspace(arguments: dict[str, Any]) -> ToolResult:
    ...
    for name in _SECTION_ORDER:
        ...
        if name == "library":
            ...
            section = _database_section(name, query, db.search_agent_workspace_rss, _rss_item)
        elif name == "downloads":
            section = _database_section(name, query, db.search_agent_workspace_downloads, _download_item)
        elif name == "organize":
            section = _database_section(name, query, db.search_agent_workspace_organize, _organize_item)
        else:
            section = _database_section(name, query, db.search_agent_workspace_local_media, _local_media_item)
        sections.append(section)
    ...
    available = sum(section["status"] in {"ready", "empty", "partial"} for section in sections)
    if available == 0:
        ok = False
        status = "unavailable"
        summary = "工作区数据源暂时不可用"
