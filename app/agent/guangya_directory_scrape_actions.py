    ...
    *,
    ...
    if delete_persisted and _repository is not None:
        try:
            ...
            logger.warning("Agent 光鸭刮削上下文清理失败 type=%s", type(exc).__name__)
    return removed
...
def _preview_safe_payload(value: Any) -> dict[str, Any] | None:
    ...
    result = {
        ...
        "candidate_title": sanitize_public_text(
            value.get("candidate_title"), limit=160
        ),
...
def _durable_flow_payload(item: _Flow) -> dict[str, Any]:
    ...
    return {
        ...
        "preview_arguments": dict(item.preview_arguments)
        if item.preview_arguments
        else None,
...
def _flow(owner: str) -> _Flow | None:
    ...
    if _repository is not None:
        try:
            ...
            logger.warning("Agent 光鸭刮削上下文恢复失败 type=%s", type(exc).__name__)
            with _lock:
...
def _save(item: _Flow) -> bool:
    ...
    if _repository is not None:
        try:
            ...
            logger.warning("Agent 光鸭刮削上下文持久化失败 type=%s", type(exc).__name__)
            with _lock:
                _flows.pop(item.owner, None)
            return False
    with _lock:
        expired = [
            owner
            for owner, row in _flows.items()
            if time.monotonic() - row.updated_at > _TTL_SECONDS
        ]
...
def _consume_flow(item: _Flow) -> bool:
    if _repository is not None:
        ...
        try:
            ...
            logger.warning("Agent 光鸭刮削上下文消费失败 type=%s", type(exc).__name__)
            return False
        if not consumed:
            return False
    with _lock:
...
def _ensure_inspection(service: Any, owner: str, flow: _Flow) -> str:
    ...
    if not flow.scope_id or flow.scope_type not in {"directory", "file"}:
        raise AgentToolError(
            "最近刮削检查无法恢复，请重新检查", code="precondition_failed"
        )
    payload = (
    ...
    if not _save(flow):
        raise AgentToolError(
            "刮削流程已被更新请求取代，请重新检查", code="precondition_failed"
        )
...
def directory_scrape_preview_arguments(arguments: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(arguments, dict) or set(arguments) - {
        ...
        raise AgentToolError(
            "只接受 candidate_number、season、episode 和 numbering_mode"
        )
    candidate = arguments.get("candidate_number")
    if (
        isinstance(candidate, bool)
        or not isinstance(candidate, int)
        or not 1 <= candidate <= 10
    ):
...
def inspect_directory_scrape(
    ...
    try:
        ...
        if "path" in resolved_arguments:
            ...
            try:
                target = resolve_workspace_path(client, resolved_arguments.pop("path"))
            finally:
                client.close()
            resolved_arguments = {
    ...
    if not _save(flow):
        raise AgentToolError(
            "本次刮削检查已被更新请求取代，请重新检查", code="precondition_failed"
        )
...
def search_directory_scrape(
    arguments: dict[str, Any], context: ToolContext
) -> ToolResult:
    flow = _flow(context.owner)
    if flow is None or not flow.inspection_id:
        raise AgentToolError(
            "最近刮削检查不存在或已过期，请先检查目录或文件", code="precondition_failed"
        )
    service = get_directory_scrape_service()
    try:
        ...
        public.append(
            {
                ...
                if key not in {"external_id", "tmdb_id"}
            }
        )
    ...
    if not _save(flow):
        raise AgentToolError(
            "本次匹配搜索已被更新请求取代，请重新搜索", code="precondition_failed"
        )
    return ToolResult(
        True,
        "completed" if public else "empty",
        f"找到 {len(public)} 个刮削匹配候选"
        if public
        else "没有找到可用的刮削匹配候选",
        data={
            "candidates": public,
            "candidate_count": len(public),
            "limits": {"max_candidates": 10, "ttl_seconds": int(_TTL_SECONDS)},
        },
        evidence=[
            Evidence(
                "metadata_search",
...
def preview_directory_scrape(
    arguments: dict[str, Any], context: ToolContext
) -> ToolResult:
    flow = _flow(context.owner)
    if flow is None or not flow.inspection_id or not flow.candidates:
        raise AgentToolError(
            "最近刮削候选不存在或已过期，请先搜索匹配", code="precondition_failed"
        )
    candidate = next(
        (
            item
    ...
    if not _save(flow):
        raise AgentToolError(
            "本次刮削预览已被更新请求取代，请重新生成", code="precondition_failed"
        )
    return ToolResult(
        True,
        "completed",
        ...
        data={
            **safe,
            "continuation_available": bool(flow.preview_id),
            "limits": {"ttl_seconds": int(_TTL_SECONDS)},
        },
        evidence=[
            Evidence(
...
def _ensure_preview_for_run(service: Any, owner: str, flow: _Flow) -> Any:
    ...
    payload = _preview_payload(
        ...
        owner,
        flow,
        flow.selected_candidate,
        flow.preview_arguments,
    )
    ...
    return confirmation_context_fingerprint(
        {
            ...
            "inspection_fingerprint": str(getattr(inspection, "fingerprint", "") or ""),
...
def execute_durable_directory_scrape_job(
    ...
    if candidate["provider"] == "metatube":
        preview_kwargs.update(provider="metatube", external_id=candidate["external_id"])
        tmdb_id = ""
    else:
        tmdb_id = candidate["tmdb_id"] or candidate["external_id"]
    if cancel_check is not None:
    ...
    if _preview_execution_guard(rebuilt_safe) != _preview_execution_guard(
        flow.preview_safe
    ):
        raise AgentToolError("刮削预览已变化，请重新预检", code="confirmation_stale")
...
def prepare_run_directory_scrape(
    ...
    if flow is None or not flow.preview_id or not flow.preview_safe:
        raise AgentToolError(
            "最近刮削预览不存在或已过期，请重新生成预览", code="precondition_failed"
        )
    try:
        service = get_directory_scrape_service()
...
def run_directory_scrape_confirmed(
    ...
    operation_ref = (
        ...
        if re.fullmatch(r"[0-9a-f]{32}", internal_task_id)
        else ""
    )
    return ToolResult(
        True,
        ...
        evidence=[
            Evidence(
                "organize_queue",
                "已提交到可恢复的光鸭整理互斥队列；仅返回可查询的操作 ID，不返回文件名或路径。",
                _now(),
            )
        ],
