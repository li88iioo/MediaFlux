...
def _store_snapshot(snapshot: _MappingSnapshot) -> None:
    with _lock:
...
def get_discovery_mapping_candidates(arguments: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    _store_snapshot(_MappingSnapshot(
...
def confirm_discovery_mapping_confirmed(
    ...
    with _lock:
        _recent.pop(context.owner, None)
    return ToolResult(
        True,
        "completed",
