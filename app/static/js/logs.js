...
async function searchOrganizeTmdb(){
    if(!organizeDetail)return;const query=document.getElementById('organizeTmdbQuery').value.trim();const year=document.getElementById('organizeTmdbYear').value.trim();const media_type=document.getElementById('organizeTmdbType').value;_setOrganizeState('正在搜索 TMDB...');
    try{const response=await api(`/api/logs/organize/${organizeDetail.id}/tmdb/search`,{method:'POST',body:JSON.stringify({query,year,media_type})});const data=await response.json();if(!response.ok)throw new Error(data.error||'搜索失败');renderOrganizeCandidates(data.candidates||[]);_setOrganizeState(data.candidates?.length?'请选择候选并预览命名':'没有找到候选',data.candidates?.length?'success':'error');}catch(error){_setOrganizeState(error.message,'error');}
}
function renderOrganizeCandidates(candidates){const box=document.getElementById('organizeTmdbCandidates');box.replaceChildren();candidates.forEach(candidate=>{const row=document.createElement('button');row.type='button';row.className='candidate-item organize-candidate';row.innerHTML=`<div class="candidate-info"><strong>${_esc(candidate.title||candidate.tmdb_id)}</strong><span class="text-muted">${_esc(candidate.year||'-')} · tmdb-${_esc(candidate.tmdb_id)}</span></div><span class="tag-mini">${Math.round((candidate.score||0)*100)}%</span>`;row.addEventListener('click',()=>selectOrganizeCandidate(candidate,row));box.appendChild(row);});}
function syncOrganizePositionFields(){const isTv=document.getElementById('organizeTmdbType').value==='tv';document.getElementById('organizePositionFields').hidden=!isTv;}
function organizeCandidatePayload(candidate=selectedOrganizeCandidate){if(!candidate)return null;const payload={...candidate};if(payload.media_type==='tv'){const season=document.getElementById('organizeSeasonOverride').value.trim();const episode=document.getElementById('organizeEpisodeOverride').value.trim();if(season!=='')payload.season=Number(season);if(episode!=='')payload.episode=Number(episode);}return payload;}
...
async function runOrganizeReorganize(){if(!selectedOrganizeCandidate){_setOrganizeState('请先选择一个 TMDB 候选并检查预览','error');return;}const payload=organizeCandidatePayload();const position=payload.media_type==='tv'?[payload.season!==undefined?`S${String(payload.season).padStart(2,'0')}`:'',payload.episode!==undefined?`E${String(payload.episode).padStart(2,'0')}`:''].filter(Boolean).join(''):'';const confirmed=await appConfirm({title:'重新整理媒体组',message:`按 ${payload.title||payload.tmdb_id}${position?` · ${position}`:''} 重新整理整个媒体组。`,confirmText:'执行重新整理',danger:true});if(!confirmed)return;try{await runOrganizeAction('/reorganize','POST',payload);}catch(error){_setOrganizeState(error.message,'error');}}
